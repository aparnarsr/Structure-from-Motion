# -*- coding: utf-8 -*-
"""
Created on Thu Oct  10 15:07:01 2019
...
class SFMSolver(object):
    ...
    def detect_and_match_feature(self, img1, img2):
        ...
        #Computing local features using SIFT
        sift = cv2.xfeatures2d.SIFT_create()
        kp1, des1 = sift.detectAndCompute(img1, None) #computn
        kp2, des2 = sift.detectAndCompute(img2, None)
        
        #Using FLANN to get global matches
        ...
        matches_good = []
        for m,n in matches:
            if m.distance < 0.7 * n.distance:  #Taking k=0.7 as it gave a better output appearance
    ...
    def compute_pose(self, p1, p2, E):
        ...
        print(" Rotation matrix_12 ",R)
        print(" Translation vector_12 ",trans)
    ...
    def triangulate(self, p1, p2, R, trans, mask):
        ...
        #Using mask to remove outliers
        matchesMask = mask.ravel().tolist() 
        p1 = p1[np.asarray(matchesMask)==1,:,:]
        p2 = p2[np.asarray(matchesMask)==1,:,:]
        
        #Normalization before triangulation
        P1norm = cv2.undistortPoints(p1, self.intrinsic,None)
        P2norm = cv2.undistortPoints(p2, self.intrinsic,None)
        
        #Computing projection (camera) matrices
        proj1 = np.concatenate((np.identity(3),np.zeros((3,1))),axis=1) #Taking no rotation and translation for the first camera
        print(" Proj1 ",proj1)
        proj2 = np.concatenate((R,trans),axis=1)
        print(" Proj2 ",proj2)
        p4d = cv2.triangulatePoints(proj1, proj2, P1norm, P2norm)
        p4d = p4d / np.tile(p4d[-1,:],(4,1))  #Converting homogeneous to output coordinates
        point_3d = p4d[:3,:].T
...
def main():
    img_pattern = 'rdimage.???.ppm'
    intrinsic = intrinsic_reader('intrinsics.txt')
    output_dir = 'output'
